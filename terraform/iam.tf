...
# Step 5: SSM Parameter Read Policy (for fetching DB password at boot)
resource "aws_iam_policy" "ssm_parameter_read_policy" {
  name        = "ssm-db-password-read-policy"
  description = "Allow EC2 to read the DB password from SSM Parameter Store"
...
resource "aws_iam_role_policy_attachment" "ssm_parameter_read_attachment" {
  role       = aws_iam_role.ec2_role.name
  policy_arn = aws_iam_policy.ssm_parameter_read_policy.arn
}

# Step 6: Create the IAM Instance Profile
